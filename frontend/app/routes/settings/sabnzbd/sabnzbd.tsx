        ...
        <div className={styles.container}>
            ...
            <Form.Group>
                <Form.Check
                    ...
                    checked={config["api.sample-filter-enabled"] === "true"}
                    onChange={e => setNewConfig({ ...config, "api.sample-filter-enabled": "" + e.target.checked })} />
                <Form.Text id="sample-filter-help" muted>
                    Discards video files named `sample` that are smaller than 20% of the largest video in the same nzb, so no webdav entry, symlink or *.strm file is created for them. The size check keeps a real release whose title contains the word from being filtered. Defaults to off.
                </Form.Text>
            </Form.Group>
            <hr />
            <Form.Group>
