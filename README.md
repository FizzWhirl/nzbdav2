...
*   **Reliability**: Large health checks now use an adaptive timeout based on segment count and health-check connection count, with a new configurable minimum timeout in Repairs settings, so 30GB+ STAT checks are not forced through the old fixed 30-minute limit.
