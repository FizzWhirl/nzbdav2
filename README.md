...
*   **UI**: Health Check Queue rows now keep active checks at the top, followed by overdue/due checks and then future scheduled checks, so the next item to start is visible immediately.
