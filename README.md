...
## v0.6.Z (2026-05-02)
*   **Revert**: Roll back commits `4a79e1ae` (lazy MP4-layout backfill from streaming + 24 h Arr-import grace bypass on stream truncation) and `67b1316a` (file-details modal Streaming Tier / MP4 layout surface) to bisect a regression where rclone WebDAV clients started receiving `401 Unauthorized` on every `PROPFIND` after these commits were deployed. Earlier GD-cap reliability work (`d7a2c6c2`, `f4e0a5e9`, `7c6007f3`, `85dc8c3f`) is retained. The `9c8b143a` Serilog noise filter and optional GD-field validation also remain.
