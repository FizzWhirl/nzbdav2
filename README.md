...
*   **UI**: Cleaned up Health page controls: the queue filters now use checkboxes instead of switches, Health empty states/actions use lucide icons instead of emoji glyphs, the queue search matches the simpler Queue page input style, and Analysis History's "Show Failed Only" label stays on one line.
*   **UI**: Normalised the Missing Articles "Blocked Only" checkbox so it matches the plain checkbox treatment used by the other filter controls.
*   **UI**: Removed redundant dark inset backgrounds from Stats data sections so Real-time Provider Status, Mapped Files, Missing Articles, and Deleted Files use the full available width; the Queue search and Add NZB controls now stay on the same row on narrow screens.
*   **UI**: Kept the Queue "Add NZB" action text on one line and made the System Logs console preserve the user's scroll position when new log entries arrive unless it is already pinned to the latest entries.
*   **Health**: Manual single-item health checks now schedule quick STAT checks by default instead of accidentally starting long HEAD scans; explicit bulk HEAD checks remain available, and active health checks now show as "Checking" immediately in the queue.
*   **Health**: HEAD health checks now use the smart header-analysis path and fall back to quick STAT existence checks when header inference is inconclusive, avoiding full BODY header scans; a timed-out urgent HEAD check is demoted to an immediate quick STAT check instead of looping through another 30-minute HEAD retry.
*   **Fix**: Confirmed DMCA/takedown health-check failures now enter the same repair workflow as missing-article failures instead of leaving the file mounted with only an action-needed health result.
*   **Feature**: Added a Repairs setting for concurrent health-check items. The scheduler can now process more than one file at a time while sharing the configured health-check connection limit across active checks.
...
*   **UI**: Health Check Queue rows now keep active checks at the top while preserving the backend scheduler order for everything else, and the release date label no longer implies it is the queue-added date.
*   **Fix**: Repairs now explicitly trigger Sonarr/Radarr replacement searches after deleting the failed media file, even when the original grab was successfully marked failed/blocklisted in Arr history, and any remaining symlink/strm link is safely removed.
*   **Reliability**: Large health checks now use a conservative adaptive timeout based on segment count and health-check connection count, with a new configurable minimum timeout in Repairs settings, so 30GB+ STAT checks are not forced through the old fixed 30-minute limit.
*   **Reliability**: Analysis runs now treat files removed by concurrent health repair as skipped instead of surfacing EF optimistic-concurrency errors, and repaired/deleted health checks no longer log as healthy completions.
*   **UI**: Analysis History now backfills incorrect file-name-as-job-name rows from the DavItem or latest health-check path, supports filtering by history type, hides Re-Analyze for removed items, and explains removed-file detail lookups in the modal.
*   **UI**: Removed Analysis History rows now short-circuit file-detail loading with a removed-file explanation, the removed action chip was removed, Action Needed Only filtering was added, and Health Check Queue initial load now includes active-check state to avoid rows jumping after WebSocket startup.
*   **Health**: Health-check result messages and File Details now show whether STAT, HEAD, or HEAD-to-STAT fallback was used; successful HEAD checks clear stale missing-article diagnostics, while successful STAT checks are shown as article-metadata checks that do not erase historical streaming/body failures.
*   **Fix**: Confirmed DMCA/takedown failures found during NZB analysis now mark the file corrupted, queue immediate repair/removal with the original takedown reason, and bypass the pending Sonarr/Radarr import grace gate just like confirmed health-check takedowns.
*   **Reliability / Diagnostics**: Large health-check timeouts now account for both segment count and virtual file size, and HEAD-to-STAT fallback history messages include the specific reason smart HEAD analysis was inconclusive; sample-probe timeouts fall back to STAT instead of failing the whole HEAD health check.
*   **UI**: Analysis History now returns total counts from the backend and uses the same Bootstrap first/previous/current/next/last pagination style as the other paged tables.
*   **Fix**: Corrupted or urgent files remain visible in the Health Check Queue even while still linked to a pending Arr history item, and the default queue view includes media files plus corrupted/ASAP rows instead of hiding them behind Show All.
*   **Health / Queue**: Import-time ffprobe analysis no longer stamps files as health-checked. Only the Step 3 article probe records a health-check timestamp, and it now creates a matching HEAD health-check result with repair status and explanatory message so File Details has complete context.
*   **UI**: Missing Articles now distinguishes definitive missing-article evidence from historical stream truncation diagnostics, shows the latest healthy STAT/HEAD context when present, and no longer labels truncation-only evidence as broken.
*   **Fix**: Remove Orphaned Files now treats "Dry Run - Done/Failed/Aborted" as a completed task state so both the live-run and dry-run buttons re-enable after a dry run finishes.
...
*   **Reliability**: Buffered stream urgent straggler retries are now bounded and coalesced by segment index, with active fetch attempts tracked independently so duplicate races cannot overwrite or prematurely clear each other's state.
*   **Performance**: Smart article probe segment-size results are now persisted for normal NZB files, and known RAR/7z/multipart part segment sizes are carried into multipart metadata so future streams can seek and prefetch with exact segment boundaries instead of re-discovering them lazily.
*   **Reliability**: RAR header extraction is now capped by a global six-slot header connection budget with at most two buffered connections per archive part, preventing large multi-part archives from multiplying part concurrency into playback-starving Usenet load.
*   **Reliability**: Missing-article summaries now keep a compact per-segment provider-evidence bitset and provider-error buffering flushes during shutdown, improving all-provider blocking detection across batches without restoring high-volume event persistence.
*   **Fix**: Health-check timeout and unexpected-error failure paths now update the `DavItem` state and insert the matching `HealthCheckResult` in a single tracked save, avoiding scheduler/UI drift from split `ExecuteUpdateAsync` plus later result writes.
*   **Diagnostics**: Added `GET /api/stats/stale-history-links` to surface media items that still have `HistoryItemId` links after the import grace period, failed history rows, or missing history rows so health-check exclusions cannot hide stale mounts indefinitely.
*   **Reliability**: Bandwidth samples and provider-affinity counters now snapshot pending data and acknowledge it only after a successful database save, preventing transient SQLite failures from silently dropping observability or provider-selection feedback.
*   **Reliability**: Added a bounded supervised background task queue and moved analysis jobs, ffprobe retries, manual repair requests, startup provider-error backfills, and missing-article health triggers onto it so failures are logged and shutdown can drain or cancel work predictably.
