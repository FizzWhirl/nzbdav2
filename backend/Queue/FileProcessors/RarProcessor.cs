...
) : BaseProcessor
{
    ...
    public override async Task<BaseProcessor.Result?> ProcessAsync()
    {
        ...
        //
        // The old fixed budget of 6 connections at 2 per volume — 3 volumes at a time,
        // process-wide — existed because header reads were buffered and each volume prefetched
        ...
        // makes the connection budget the binding limit rather than memory.
    ...
    private async Task<List<StoredFileSegment>> ProcessPartAsync(GetFileInfosStep.FileInfo fileInfo, CancellationToken processorToken)
    {
        ...
        if (fileInfo.SegmentSizes == null)
        {
            try
            {
                ...
                if (partIds.Length > 0)
                {
                    var computed = await usenet.AnalyzeNzbAsync(partIds, SegmentAnalysisConnectionsPerPart, null, headerCts.Token, useSmartAnalysis: true).ConfigureAwait(false);
